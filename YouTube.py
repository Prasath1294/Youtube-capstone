...
client = pymongo.MongoClient("mongodb://Guvi1234:Guvi1234@ac-zpyq7xu-shard-00-00.6f1kxmr.mongodb.net:27017,ac-zpyq7xu-shard-00-01.6f1kxmr.mongodb.net:27017,ac-zpyq7xu-shard-00-02.6f1kxmr.mongodb.net:27017/?ssl=true&replicaSet=atlas-2ayk4o-shard-0&authSource=admin&retryWrites=true&w=majority")
mg_db = client['Channel_Database']
ram = psycopg2.connect(host="localhost", user="postgres", password="Ram@1294", port=5432, database="Youtubedata")
guvi = ram.cursor()
...
channel_id = 'UCokRlR4P9kINaTq9_v5DFhQ' # Chathurbujan Veedu
...
st.set_page_config(page_title="YouTube Data Harvesting and Warehousing", page_icon="", layout="wide", initial_sidebar_state="expanded")
st.markdown(
    """
    <style>
    .main {
...
with st.sidebar:
    selected = option_menu(
        ...
        options = ["Home","---","Harvest","Migrate","FAQ","---","About"],
        icons=["house","","arrow-move","patch-question","","envelope"],
        menu_icon="youtube",
        default_index=0,
...
if selected == 'Home':
    ...
    col1, col6, col7 = st.colums([2,1,2])
    with col1:
        channel_id = st.text_input("Enter Channel Id:")
    col4, col5, col6, col7 = st.colums([1,1,2,2])
    with col4:
    ...
    if Show:
        ...
        if dataframe:
            ...
            with tab3:
                ...
                y = guvi.fetchall()
                comment_df_mig = pd.DataFrame(y, columns=['Video Name', 'Comment', 'Author', 'Commented On', 'Likes', 'replies'])
    ...
    with st.expander("Q2. Which channels have the most number of videos, and how many videos do they have?"):
        query2 = "slect channel_name, total_videos as Videos from channel_table order by total_videos desc limit 3"
        guvi.execute(query2)
        faq2 = guvi.fetchall()
        faq2_df = pd.DataFrame(faq2,columns=['Channel Name','Total Videos'])
    ...
    with st.expander("Q3. What are the top 10 most viewed videos and their respective channels?"):
        query3 = "select channel_table.channel_name, video_table.title, video_table.view_count from video_table join channel_table " \
                 "on video_table.channel_id = channel_table.channel_id order by video.table.view_count desc limit 10"
        guvi.execute(query3)
        faq3 = guvi.fetchall()
    ...
    with st.expansder("Q6. What is the total number of likes and dislikes for each video and what are their corresponding video names?"):
