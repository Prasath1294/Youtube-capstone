...
# MongoDB Connection
client = MongoClient("mongodb://Guvi1234:Guvi1234@ac-zpyq7xu-shard-00-00.6f1kxmr.mongodb.net:27017,"
                     "ac-zpyq7xu-shard-00-01.6f1kxmr.mongodb.net:27017,ac-zpyq7xu-shard-00-02.6f1kxmr.mongodb.net:27017"
                     "/?ssl=true&replicaSet=atlas-2ayk4o-shard-0&authSource=admin&retryWrites=true&w=majority")
db = client['Youtube_project']

channel_list = db.list_collection_names()

ram = psycopg2.connect(host="localhost", user="postgres", password="Ram@1294", port=5432,
                               database="Youtubedata")
guvi = ram.cursor()

api_key = "AIzaSyBxWtaA8TnkVB1AjbGsMwfC5GW07PQEptQ"
youtube = build("youtube", "v3", developerKey=api_key)
channel_ids = ["UC-khaI-PA5NJmGx6xY2hLDw", "UCYnFjolfAkX5n9ExbTobjPQ"]
...
def video_comments(playlist_id):
    video_ids_list = get_video_stats(playlist_id)
    comment_info = []
...
with st.sidebar:
    opt = option_menu(menu_title='Menu',
                      ...
                      icons=['house', 'uploads', 'table', 'check2-square'],
                      menu_icon='cast',
                      default_index=0)
if opt == "About":
    ...
    if migrate:
        ...
        channel_df["channel_view_count"] = channel_df["channel_view_count"].astype(int)
        # print(channel_df)
        guvi.execute(
        ...
        for index, value in channel_df.iterrows():
            ...
            guvi.execute(f"SELECT * FROM CHANNEL WHERE CHANNEL_ID ='{val}'")
            w = guvi.fetchall()
            if len(w) > 0:
                guvi.execute(f"DELETE FROM CHANNEL WHERE CHANNEL_ID = '{val}'")
                ram.commit()
            result_1 = (
                value['channel_id'], value['channel_name'], value['subscriber_count'], value["channel_view_count"],
                value["channel_des"])
            guvi.execute(a, result_1)
        ram.commit()
        ...
        video_df['video_duration'] = video_df['video_duration'].dt.total_seconds().astype(int)
        ...
        for index, value_2 in video_df.iterrows():
            ...
            guvi.execute(f"SELECT * FROM VIDEO WHERE VIDEO_ID = '{val_2}'")
            y = guvi.fetchall()
            if len(y) > 0:
                guvi.execute(f"DELETE FROM VIDEO WHERE VIDEO_ID = '{val_2}'")
                ram.commit()
            result_3 = (
            ...
            guvi.execute(c, result_3)
        ram.commit()

        comments_dict = video_comments(channel_dict['playlist_id'])
        comments_df = pd.DataFrame(comments_dict)
        comments_df['published_at'] = pd.to_datetime(comments_df['published_at']).dt.date.astype("datetime64[ns]")
        print(comments_df.info())
        print(comments_df.head(2))
        guvi.execute(
            "CREATE TABLE IF NOT EXISTS COMMENTS(VIDEO_ID VARCHAR(75), COMMENT_ID VARCHAR(75) PRIMARY KEY, "
            "PUBLISHED_AT TIMESTAMP, COMMENTED_BY VARCHAR(50), COMMENT_GIVEN TEXT)")
        ram.commit()
        ...
        for index, value_3 in comments_df.iterrows():
            ...
            guvi.execute(f"SELECT * FROM COMMENTS WHERE COMMENT_ID = '{val_3}'")
            z = guvi.fetchall()
            if len(z) > 0:
                guvi.execute(f"DELETE FROM COMMENTS WHERE COMMENT_ID = '{val_3}'")
                ram.commit()
            result_4 = (
                value_3["video_id"], value_3["comment_id"], value_3["published_at"], value_3["author_display_name"],
                value_3["display_text"])
            guvi.execute(d, result_4)
        ram.commit()

else:
    st.title("YouTube Data Harvesting and Warehousing with MongoDB, SQL")
    st.subheader("Simplify and analyze the migrated data from the questions to gain valuable insights")
    ...
    question_list = ["Select", "What are the names of all the videos and their corresponding channels?",
                     ...
                     "channel names?",
                     "Which videos have the highest number of comments, and what are their corresponding channel "
                     "names?"]
    ...
    if question_selected == "What are the names of all the videos and their corresponding channels?":
        guvi.execute(
            "SELECT CHANNEL.CHANNEL_NAME, VIDEO.VIDEO_TITLE FROM CHANNEL JOIN VIDEO ON VIDEO.CHANNEL_ID = "
            "CHANNEL.CHANNEL_ID")
        fetch = guvi.fetchall()
    ...
    if question_selected == "Which channels have the most number of videos, and how many videos do they have?":
        guvi.execute("SELECT CHANNEL.CHANNEL_NAME, COUNT(VIDEO_ID) AS VIDEO_COUNT FROM VIDEO JOIN CHANNEL ON "
                            "CHANNEL.CHANNEL_ID = VIDEO.CHANNEL_ID GROUP BY CHANNEL.CHANNEL_ID, VIDEO.CHANNEL_ID ORDER "
                            "BY VIDEO_COUNT DESC LIMIT 3")
        fetch = guvi.fetchall()
    ...
    if question_selected == "What are the top 10 most viewed videos and their respective channels?":
        guvi.execute(
            "SELECT CHANNEL.CHANNEL_NAME, VIDEO.VIEW_COUNT FROM CHANNEL JOIN VIDEO ON VIDEO.CHANNEL_ID = "
    ...
    if question_selected == "How many comments were made on each video, and what are their corresponding video names?":
        guvi.execute(
            "SELECT VIDEO.VIDEO_TITLE, COUNT(COMMENT_ID) AS TOTAL_COMMENT FROM COMMENTS JOIN VIDEO ON "
    ...
    if question_selected == "Which videos have the highest number of likes, and what are their corresponding channel " \
                            "names?":
        guvi.execute(
            "SELECT CHANNEL.CHANNEL_NAME, VIDEO.VIDEO_TITLE, VIDEO.LIKE_COUNT FROM VIDEO JOIN CHANNEL ON "
    ...
    if question_selected == "What is the total number of likes and dislikes for each video, and what are their " \
        ...
        guvi.execute("SELECT VIDEO_TITLE, LIKE_COUNT, DISLIKE_COUNT FROM VIDEO")
        fetch = guvi.fetchall()
        df = pd.DataFrame(fetch, columns=['Video Title', 'Like Count', 'Dislike Count'])
        st.table(df)

    if question_selected == "What is the total number of views for each channel, and what are their " \
                            "corresponding channel names?":
        guvi.execute("SELECT CHANNEL_NAME, CHANNEL_VIEW_COUNT FROM CHANNEL ORDER BY CHANNEL_VIEW_COUNT DESC")
        fetch = guvi.fetchall()
    ...
    if question_selected == "What are the names of all the channels that have published videos in the year 2022?":
        guvi.execute(
            "SELECT CHANNEL_NAME FROM CHANNEL JOIN VIDEO ON CHANNEL.CHANNEL_ID = VIDEO.CHANNEL_ID WHERE "
    ...
    if question_selected == "What is the average duration of all videos in each channel, and what are their " \
                            "corresponding channel names?":
        guvi.execute(
            "SELECT CHANNEL.CHANNEL_NAME, AVG(VIDEO.VIDEO_DURATION)::INT AS AVERAGE_DURATION FROM VIDEO JOIN "
    ...
    if question_selected == "Which videos have the highest number of comments, and what are their corresponding " \
                            "channel names?":
        guvi.execute(
            "SELECT CHANNEL.CHANNEL_NAME, VIDEO.VIDEO_TITLE, VIDEO.COMMENT_COUNT FROM VIDEO JOIN CHANNEL ON "
